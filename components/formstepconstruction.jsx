...
const stepsnew = [
    { name: 'Funded', date: 'Feb 07' },
    { name: 'Closing', date: 'Feb 07' },
    { name: 'Design and Engineering', date: 'Feb 07' },
    { name: 'Interconnection and All Permits Issued', date: 'Feb 07' },
    { name: 'Module Delivery and Purchase Orders for other Major Materials', date: 'Feb 06' },
    { name: 'Mobilaztion', date: 'Feb 06' },
    { name: 'Mechanical Completion', date: 'Feb 06' },
    { name: 'PTO and Substantial Completion', date: 'Feb 06' },
    { name: 'Final Completion', date: 'Feb 07' },
...
const FormStepcon = ({ project, fetchProjectDetails }) => {
    const [currentStep, setCurrentStep] = useState(project?.currentStep || 1);
    const [epcInstallPrice, setEpcInstallPrice] = useState(project?.systemInfo?.epcInstall || 0);

    useEffect(() => {
        setCurrentStep(project?.currentStep || 1);
        setEpcInstallPrice(project?.systemInfo?.epcInstall || 0);
    }, [project?.currentStep, project?.systemInfo?.epcInstall]);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        ...
                        const response = await axios.put(`${process.env.REACT_APP_API_BASE_URL}/projects/${project._id}/update-steps`, {
                            step: nextStep,
                        });

                        if (response.data.success) {
                            setCurrentStep(nextStep);
                            Swal.fire("Success!", "Step status updated.", "success");
                        } else {
    ...
    return (
        <Box className='steps-wrapper'>
            <Box className='steps-heading'>
                <Typography component='h2'>Construction Progress</Typography>
                <Typography component='p'>EPC Installed Price: ${epcInstallPrice}</Typography>
            </Box>
            <Stepper className='form-steps' activeStep={currentStep} alternativeLabel>
                {stepsnew.map((label, index) => (
                    <Step
                        key={index}
                        className={`form-step ${index <= currentStep ? "active-step" : ""}`}
